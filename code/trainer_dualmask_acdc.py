...
from val_acdc import test_single_volume
...
def write_uncertainty_batch_heatmaps(writer, tag_prefix, uncertainty_map, iter_num, cmap='inferno', save_dir=None):
    ...
    for vis_idx in range(B):
        for cls in range(1, 4):  # 跳过背景
            ...
            im = ax.imshow(data, cmap=cmap, vmin=0.0, vmax=1.0, interpolation='nearest')
            ax.axis('off')
...
def trainer_acdc_dualmask_prompt_ssl_fixcoe_random_new_mean_up(args, model, snapshot_path, multimask_output, low_res):
    ...
    if args.AdamW:
        ...
        optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=b_lr, momentum=0.9, weight_decay=0.0001)  # Even pass the model.parameters(), the `requires_grad=False` layers will not update
    writer = SummaryWriter(snapshot_path + '/log')
    log_path = os.path.join(snapshot_path, 'case_names_log.txt') 
    iter_num = 0
    max_epoch = args.max_epochs
    stop_epoch = args.stop_epoch
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            with open(log_path, 'a') as log_file:
                for idx, name in enumerate(sampled_batch['case_name']):
                    log_file.write(f"Iter {iter_num} | Batch {i_batch} | Sample {idx} -> Case: {name}\n")
            ...
            #outputs3 = outputs['low_res_logits3']
            ...
            #supervised_loss3, loss_ce3, loss_dice3 = calc_loss_labeled(outputs3, label_batch, ce_loss, dice_loss, labeled_bs,args.dice_param)
            loss1 = supervised_loss1 + supervised_loss2 #+ supervised_loss3
            optimizer.zero_grad()
            loss1.backward()
            optimizer.step()
            ...
            if iter_num < args.warm_iter:
                ...
                if iter_num % 20 == 0:
                    with torch.no_grad():
                        write_mask_to_tensorboard(writer, tag='mask/round3_1_no_prompt', masks=outputs_round3_soft1, iter_num=iter_num, save_dir=os.path.join('output_acdc', 'mask_pngs'))
                        write_mask_to_tensorboard(writer, tag='mask/round3_2_conf_prompt', masks=outputs_round3_soft2, iter_num=iter_num, save_dir=os.path.join('output_acdc', 'mask_pngs'))
                        write_mask_to_tensorboard(writer, tag='mask/round3_2_uncertainty_prompt', masks=outputs_round3_soft2_uncertainty, iter_num=iter_num, save_dir=os.path.join('output_acdc', 'mask_pngs'))
                        write_mask_to_tensorboard(writer, tag='mask/round3_2_pseudo_label', masks=outputs_round3_soft2_po, iter_num=iter_num, save_dir=os.path.join('output_acdc', 'mask_pngs'))
                        write_uncertainty_batch_heatmaps(writer, tag_prefix='uncertainty_map', uncertainty_map=uncertainty_map, iter_num=iter_num, save_dir=os.path.join('output_acdc', 'uncertainty_pngs'))
                #'''
