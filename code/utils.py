...
def test_single_volume(case_name,image, label, net, classes, multimask_output, patch_size=[256, 256], input_size=[160, 160],
    ...
    if len(image.shape) == 3:
        ...
        # get resolution
        case_raw = '/home/xlk/SSL/new_CPC-SAM/data/ACDC_raw' + f"{case_name}.nii.gz"
    ...
    if test_save_path is not None:
        ...
        sitk.WriteImage(prd_itk, test_save_path + '/' + f"{case_name}_pred.nii.gz")
        sitk.WriteImage(img_itk, test_save_path + '/' + case + "_img.nii.gz")
        sitk.WriteImage(lab_itk, test_save_path + '/' + case + "_gt.nii.gz")
        print('saved successfully!')
    return metric_list

def test_single_volume_mean(case_name,image, label, net, classes, multimask_output, patch_size=[256, 256], input_size=[160, 160],
                       test_save_path=None, case=None, z_spacing=1):
    if len(image.shape) == 3:
        prediction = np.zeros_like(label)
        max_uncertainty_value = -1
        uncertainty_vis_slice = None
        for ind in range(image.shape[0]):
            slice = image[ind, :, :]
            x, y = slice.shape[0], slice.shape[1]
            if x != patch_size[0] or y != patch_size[1]:
            ...
            with torch.no_grad():
                ...
                prediction[ind] = pred
        # get resolution
        case_raw = '/home/xlk/SSL/new_CPC-SAM/data/ACDC_raw/labels/' + f"{case_name}.nii.gz"
        case_raw = sitk.ReadImage(case_raw)
        raw_spacing = case_raw.GetSpacing()
        raw_spacing_new = []
        raw_spacing_new.append(raw_spacing[2])
    ...
    elif len(image.shape) == 4:  #  (C, D, H, W)
        C, D, H, W = image.shape
        prediction = np.zeros_like(label)
        max_uncertainty_value = -1
        uncertainty_vis_slice = None
        for ind in range(D):
            slice = image[:, ind, :, :]  # shape: (C, H, W)
            x, y = slice.shape[1], slice.shape[2]
            if x != patch_size[0] or y != patch_size[1]:
                slice = np.stack([
                    zoom(slice[c], (patch_size[0] / x, patch_size[1] / y), order=3)
                    for c in range(C)
                ], axis=0)

            inputs = torch.from_numpy(slice).unsqueeze(0).float().cuda()  # shape (1, C, H, W)
            ...
            with torch.no_grad():
                ...
                out = torch.argmax(output_masks, dim=1).squeeze(0).cpu().numpy()
        ...
        # get resolution
        case_raw = '/home/xlk/SSL/CPC-SAM/data/brats_raw/labels/' + f"{case_name}.nii.gz"
        case_raw = sitk.ReadImage(case_raw)
        raw_spacing = list(case_raw.GetSpacing())[::-1]  # (z, y, x)
    ...
    if test_save_path is not None:
        ...
        img_to_save = image[0] if image.ndim == 4 else image  # shape: (D, H, W)
        
        img_itk = sitk.GetImageFromArray(img_to_save.astype(np.float32))
        prd_itk = sitk.GetImageFromArray(prediction.astype(np.float32))
        lab_itk = sitk.GetImageFromArray(label.astype(np.float32))
        ...
        sitk.WriteImage(prd_itk, os.path.join(test_save_path, f"{case_name}_pred.nii.gz"))
        sitk.WriteImage(img_itk,  os.path.join(test_save_path, f"{case_name}_img.nii.gz"))
        sitk.WriteImage(lab_itk, os.path.join(test_save_path, f"{case_name}_gt.nii.gz"))
        print(f" Saved: {case_name}_pred / _img / _gt to {test_save_path}")

    return metric_list

def test_single_image(image, label, net, classes, multimask_output, patch_size=[256, 256], input_size=[160, 160],
                       test_save_path=None, case=None, z_spacing=1):
    if len(image.shape) == 3:
        prediction = np.zeros_like(label)
        slice = image#[ind, :, :]
...
def test_single_image_mean(image, label, net, classes, multimask_output, patch_size=[256, 256], input_size=[160, 160],
                       test_save_path=None, case=None, z_spacing=1):
    if len(image.shape) == 3:
        prediction = np.zeros_like(label)
        slice = image#[ind, :, :]
